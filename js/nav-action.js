...
//            var    d = a('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(g).click(function () {
//                    if (f.paused) {
//                        f.play()
//                    } else {
//                        f.pause()
...
window.toggleFooter = function (hide) {
    var footer;
    footer = $("#footer");
	var id1 = $("#nextTopic span");
	/*id1.click(function (event) {
      	event.stopPropagation();
        return jQuery.scrollTo(0, scrollSpeed, {
            axis: "y",
            easing: easing
        });
    	});*/
		//$("#nextTopic").before('<span class="h">ATA Fleet View</span>');
    ...
    if (!hide) {
		...
		$("#nextTopic").prepend('<span class="last">ATA Fleet View</span>');
		$(".last").click(function(){
			window.scroll(0,0);
		})
        return id1.stop().animate({
            //height: 0,
			//opacity: 0,
				
			
        }, navSpeed, function () {
            return $(this).data("visible", "visible");
			//return id1;
        });
		
    } else {
        if (!footer.data("visible")) {
			//var id2 = $("#nextTopic").show();
			//id1.attr('class','');
			//$("#nextTopic span").after('<span class="h">Hello</span>');
			$( ".last" ).remove();
            return id1.stop().animate({
                height: 75,
                opacity: 1
...
var selectPreviousTopic = function (f) {
    $currentTopic = $(topicsSuper).find("> li.active");
    /*************Blank Hide li conditions********************/
	$hideT = $currentTopic.prev().text();;
	
//alert($hideT);
   if($hideT == ""){
	   $prevTopic = $currentTopic.prev().prev().prev().prev();
   }
   else{
